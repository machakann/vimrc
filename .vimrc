...
if neobundle#tap('vim-smartinput')
  ...
  let s:trig = [
        ...
        \     ['<Plug>(smartinput_^q)', '<Plug>(smartinput_^q)',  '<C-q>'],
        \     ['(',     '(',     '('    ],
        \     [')',     ')',     ')'    ],
        \     ['[',     '[',     '['    ],
        \     [']',     ']',     ']'    ],
  ...
  let rules += [
        ...
        \       {'char': '<Plug>(smartinput_^k)', 'at': '\[\%#\]',     'input': '<BS><Del>\[\]<Left><Left>',  'filetype': ['vim']},
        \       {'char': '<Plug>(smartinput_^k)', 'at': '<\%#>',       'input': '<BS><Del>\<\><Left><Left>',  'filetype': ['vim']},
        \       {'char': '<Plug>(smartinput_^k)', 'at': '\\%(\%#\\)',  'input': '<BS><BS><Del>(',             'filetype': ['vim']},
  ...
  " Julia langage
  let rules += [
        \       {'char': '<Plug>(smartinput_^k)', 'at': '\k\+:\%#',  'input': ':<C-r>=LocalComplete("type")<CR>'},
        \       {'char': '<Plug>(smartinput_^k)', 'at': '\k\+::\%#', 'input': '<C-r>=LocalComplete("type")<CR>'},
        \       {'char': '<Plug>(smartinput_^k)', 'at': '\%(Dict\|Array\|Vector\){\%#', 'input': '<C-r>=LocalComplete("type")<CR>'},
        \       {'char': '<Plug>(smartinput_^k)', 'at': 'convert(.*,\s*\%#', 'input': '<C-r>=LocalComplete("type")<CR>'},
        \      ]
...
function! TextobjLastchanged() abort
  ...
  let tail  = getpos("']")
  let empty = [0, 0, 0, 0]
  return head != empty && tail != empty ? ['v', head, tail] : 0
endfunction
"}}}
" textobj-number "{{{
